...
**Gateway-aware save directive.** The `<memory-rules>` block is gateway-aware, decided **programmatically at runtime from config presence** — no env var or manual flag to set. When an agent-tools gateway is configured (`memory setup gateway`), the block omits the post-action save directive (Rule B — scope classification — plus the memory quality gate) because the gateway's `tasks done` reminder owns the save nudge. When no gateway is configured, the block keeps Rule B as the fallback save rule. Re-running `memory setup rules` after configuring (or removing) a gateway updates the block accordingly.
